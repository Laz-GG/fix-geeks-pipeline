...
import {
  Prospecto, Seguimiento, TipoCaptura, CANALES, ESTADOS_ASIGNABLES, PROXIMOS_PASOS,
  ACCIONES, RESULTADOS_SEGUIMIENTO, MOTIVOS_PERDIDA, uid, nowIso, isActivo, buildMensaje,
} from "@/lib/prospectos";
...
export function ProspectoDialog({ open, onClose, prospecto, seguimientos, initialTab, onSave, onAddSeguimiento }: Props) {
  ...
  const handleSave = () => {
    ...
    const accionFinal = form.ultima_accion === "Otra"
      ? (form.accion_personalizada || "").trim()
      : form.ultima_accion;
    ...
    if (isNew && form.tipo_captura === "captura_manual") {
      if (!form.ultima_accion) errors.push("última acción");
      if (form.ultima_accion === "Otra" && !accionFinal) errors.push("acción personalizada");
      if (!resultadoInicial) errors.push("resultado inicial");
    }
  ...
  const markConvertido = () => {
    ...
    const updated: Prospecto = {
      ...
      ultima_accion: "Prospecto convertido", accion_personalizada: null,
  ...
  const markPerdido = () => {
    ...
    const updated: Prospecto = {
      ...
      ultima_accion: "Prospecto marcado como perdido", accion_personalizada: null,
  ...
  const addSeguimiento = () => {
    ...
    const accionFinal = nuevo.accion === "Otra"
      ? nuevo.accion_personalizada.trim()
      : nuevo.accion;
    if (nuevo.accion === "Otra" && !accionFinal) return toast.error("Captura la acción personalizada");

    const s: Seguimiento = {
      id: uid(), prospecto_id: form.id, fecha: nowIso(),
      canal: nuevo.canal as Seguimiento["canal"], accion: accionFinal,
  ...
  const estadosUI = (form.estado === "Convertido" || form.estado === "Perdido")
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="cierre" disabled={!prospecto}>Cierre</TabsTrigger>
            ...
            {isNew && (
              <div className="rounded-lg border bg-primary/5 p-3 space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => set("tipo_captura", v as TipoCaptura)}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="nuevo">Prospecto nuevo sin seguimiento previo</SelectItem>
                    <SelectItem value="captura_manual">Prospecto ya atendido / captura manual</SelectItem>
                  </SelectContent>
                </Select>
          ...
          <TabsContent value="comercial" className="space-y-4 pt-4">
            <div className="rounded-lg border bg-muted/40 p-3 text-xs text-muted-foreground">
              <strong>Última acción</strong> = lo que ya ocurrió. <strong>Próximo paso</strong> = lo que toca hacer.
              <br />Ej.: Última acción: <em>Se mandó ubicación</em>. Próximo paso: <em>Confirmar si vendrá hoy</em>.
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Estado *">
                <Select value={form.estado} onValueChange={v => set("estado", v as Prospecto["estado"])}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {estadosUI.map(e => <SelectItem key={e} value={e}>{e}</SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <Field label="Última acción *">
                <Select
                  value={ACCIONES.includes(form.ultima_accion as typeof ACCIONES[number]) ? form.ultima_accion : "Otra"}
                  onValueChange={v => {
                    if (v === "Otra") {
          ...
          <TabsContent value="historial" className="space-y-4 pt-4">
            <div className="rounded-lg border p-4 space-y-3 bg-muted/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <Field label="Canal">
                  <Select value={nuevo.canal} onValueChange={v => setNuevo(n => ({ ...n, canal: v }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{CANALES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                  </Select>
                </Field>
                ...
                {nuevo.accion === "Otra" && (
                  <Field label="Acción personalizada">
                    <Input value={nuevo.accion_personalizada} onChange={e => setNuevo(n => ({ ...n, accion_personalizada: e.target.value }))} />
                  </Field>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <Field label="Nuevo estado (opcional)">
                  <Select value={nuevo.nuevoEstado} onValueChange={v => setNuevo(n => ({ ...n, nuevoEstado: v }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{estadosUI.map(e => <SelectItem key={e} value={e}>{e}</SelectItem>)}</SelectContent>
                  </Select>
                </Field>
