...
function demoData(): { prospectos: Prospecto[]; seguimientos: Seguimiento[] } {
  ...
  const ps: Prospecto[] = [
    ...
    },
    {
      ...
      estado: "Nuevo", ultima_accion: "Primer mensaje recibido",
    ...
    },
    {
      ...
      estado: "Información incompleta", ultima_accion: "Pedimos modelo y fotos",
  ...
  const ss: Seguimiento[] = ps.map((p, i) => ({
    id: uid(), prospecto_id: p.id, fecha: p.updated_at,
    canal: p.canal_origen, accion: p.ultima_accion,
    comentario: ["Cliente respondió rápido", "Esperamos respuesta", "Sin respuesta", "Excelente seguimiento", "Cierre"][i] || "",
    usuario: p.persona_que_atendio, resultado: p.estado === "Convertido" ? "Convertido" : p.estado === "Perdido" ? "Perdido" : "Respondió",
  }));
...
export function exportCsv(prospectos: Prospecto[]) {
  const cols: (keyof Prospecto)[] = [
    ...
    "proximo_paso","fecha_proximo_seguimiento","fecha_conversion","referencia_orden_gestioo",
    "fecha_perdido","motivo_perdido","created_at","updated_at",
  ];
  const esc = (v: unknown) => {
